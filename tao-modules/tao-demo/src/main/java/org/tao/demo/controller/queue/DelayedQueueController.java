package org.tao.demo.controller.queue;

import cn.dev33.satoken.annotation.SaIgnore;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.tao.common.core.domain.R;
import org.tao.common.redis.utils.QueueUtils;
...
@RestController
@RequestMapping("/demo/queue/dtaoed")
public class DtaoedQueueController {
    ...
    public R<Void> add(String queueName, String orderNum, Long time) {
        QueueUtils.addDtaoedQueueObject(queueName, orderNum, time, TimeUnit.SECONDS);
        // 观察发送时间
    ...
    public R<Void> remove(String queueName, String orderNum) {
        if (QueueUtils.removeDtaoedQueueObject(queueName, orderNum)) {
    ...
    public R<Void> destroy(String queueName) {
        // 用完了一定要销毁 否则会一直存在
        QueueUtils.destroyDtaoedQueue(queueName);
        return R.ok("操作成功");
    }
