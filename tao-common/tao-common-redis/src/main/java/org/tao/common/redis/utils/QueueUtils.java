package org.tao.common.redis.utils;
...
public class QueueUtils {
     ...
     * @param time      延迟时间
     */
    public static <T> void addDtaoedQueueObject(String queueName, T data, long time) {
        addDtaoedQueueObject(queueName, data, time, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit  单位
     */
    public static <T> void addDtaoedQueueObject(String queueName, T data, long time, TimeUnit timeUnit) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDtaoedQueue<T> dtaoedQueue = CLIENT.getDtaoedQueue(queue);
        dtaoedQueue.offer(data, time, timeUnit);
    }
     ...
     * @param queueName 队列名
     */
    public static <T> T getDtaoedQueueObject(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDtaoedQueue<T> dtaoedQueue = CLIENT.getDtaoedQueue(queue);
        return dtaoedQueue.poll();
    }

    /**
     * 删除延迟队列数据
     */
    public static <T> boolean removeDtaoedQueueObject(String queueName, T data) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDtaoedQueue<T> dtaoedQueue = CLIENT.getDtaoedQueue(queue);
        return dtaoedQueue.remove(data);
    }

    /**
     * 销毁延迟队列 所有阻塞监听 报错
     */
    public static <T> void destroyDtaoedQueue(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDtaoedQueue<T> dtaoedQueue = CLIENT.getDtaoedQueue(queue);
        dtaoedQueue.destroy();
    }
     ...
     */
    public static <T> void subscribeBlockingQueue(String queueName, Function<T, CompletionStage<Void>> consumer, boolean isDtaoed) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        if (isDtaoed) {
            // 订阅延迟队列
            CLIENT.getDtaoedQueue(queue);
        }
